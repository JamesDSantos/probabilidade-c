...
a) Construa o grafo desta cadeia de Markov com os estados {0, 1, 2}.

b) Determine as probabilidades de transição $p_{02}$ e $p_{10}$.
:::
...
a) Desenhe o grafo da cadeia.

b) Identifique as classes.

c) Classifique cada classe como transiente ou recorrente e encontre seu período.
:::

::: {#exr-}
Determine o período ($d_i$) do estado 0 para as seguintes situações:

a) $p_{01}=1, p_{12}=1, p_{20}=1$.

b) $p_{01}=1, p_{10}=0,5, p_{12}=0,5, p_{20}=1$.

c) O estado 0 possui um arco para si mesmo ($p_{00} > 0$). Qual é o seu período?
:::

::: {#exr-}
Em uma cidade, o clima pode ser Seco (0) ou Chuvoso (1). As probabilidades de transição são

$$\begin{array}{c|cc}\hline i\setminus j & 0 & 1 \\\hline  0 &0,8 & 0,2 \\ 1&0,4 & 0,6 \\ \hline\end{array}$$

a) Calcule a distribuição estacionária $(\pi_0, \pi_1)$.

b) Um vendedor de sorvetes lucra R\$200,00 em dias secos e perde R\$ 50,00 em dias chuvosos. Qual o lucro médio diário esperado no longo prazo?
:::


::: {#exr-}
Três operadoras de celular (A, B e C) disputam o mercado.  As probabilidades de transição mensal dos clientes são entre as operadoras são:
