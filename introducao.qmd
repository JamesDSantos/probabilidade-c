...
Duas máquinas do mesmo tipo são ligadas simultaneamente. Seja $\textbf{X}=(X_1,X_2)$
 ...
 é o tempo de vida da $i$-ésima máquina). Suponha que $X_i\sim$Exponencial($\lambda$), com $\lambda>0$
 conhecido e que $X_1$ é independente de $X_2$. 
 
 A probabilidade de que ambas as máquinas estejam funcionando até o tempo $\textbf{x}=(x_1,x_2)$
 é dada por

$$\begin{align}P(X_1>x_1,X_2>x_2)&=P(X_1>x_1)P(X_2>x_2)\\&=\left(\int_{x_1}^\infty \lambda e^{-\lambda y_1}dy_1\right)\left(\int_{x_2}^\infty \lambda e^{-\lambda y_2}dy_2\right)\\&=\int_{x_1}^\infty\int_{x_2}^\infty \lambda^2 e^{-\lambda(y_1+y_2)}dy_1dy_2,\end{align}$$
e, portanto,
$$f(\textbf{x})=λ^2e^{−λ(x_1+x_2)}$$
para $x_1,x_2>0$ é a densidade conjunta de $\textbf{X}=(X_1,X_2)$.
:::

<div class='alert alert-success'>
**Proposição 2** Dizemos que $f(\textbf{x})$ (contínua) é a função de densidade para um vetor aleatório $\textbf{X}$ se
...
### 1.2.3 Função de distribuição conjunta
Seja $\textbf{X}$ um vetor de variáveis aleatórias de qualquer natureza. A função
...
<div class='alert alert-success'>
**Proposição 3.** A função de distribuição de um vetor de variáveis aleatórias discretas é dada por

$$F(\textbf{x})=\sum_{y_1=−\infty}^{x_1}\cdots \sum_{y_n=−\infty} ^{x_n}P(X_1=y_1,\ldots,X_n=y_n),$$
e a função de distribuição de um vetor de variáveis aleatórias contínuas é dada por
...
$$\begin{align}F(x,y)&=\int_0^x\int_0^y \frac{3}{80}(u^2+uv)dvdu\\&=\int_0^x\left[\frac{3}{80}(u^2+\frac{uy^2}{2})\right]du\\ &=\frac{x^3}{80}+\frac{3x^2y^2}{320},\end{align}$$
se $0<x<2$ e $0<y<4$.
:::

<div class='alert alert-warning'>
**Exercício 5.** Considere o vetor $(X,Y)$
 de variáveis aleatórias com função densidade conjunta dada por
$$f(x,y)=xe^{−xy},$$
com $y>0$ e $x\in(0,1)$. Encontre a função de distribuição conjunta de $(X,Y)$.
</div>

<div class='alert alert-success'>
**Proposição 4**. Se $\textbf{X}$
...
Como em geral utilizamos as letras $F$, $P$ e $f$ para designar as funções de distribuição, probabilidade e densidade, é importante acrescentar uma notação de identifique explicitamente a variável. Quando isto for necessário, adicionaremos a letra que designa a variável subscrita na função. Por exemplo, $F_{X_1}(x)$ é a função de distribuição marginal de $X_1$.




<div class='alert alert-success'>
**Proposição 5.** Seja $\textbf{X}$ um vetor de variáveis aleatórias de comprimento $n$. A função de distribuição marginal de $X_i$ é dada por

$$F_{X_i}(x_i)=\lim_{\textbf{x}_{(−i)}\rightarrow\infty}F(\textbf{x}),$$
onde $\textbf{x}_{(−i)}=\textbf{x}\setminus x_i$ (ou seja, o vetor $\textbf{x}$ sem a $i$-ésima coordenada).
</div>

<div class='alert alert-warning>
**Exercício 8.** Considere a seguinte função de distribuição conjunta,

$$F(x,y)=1−e^{−x}−e^{−y}+e^{−(x+y)}.$$
onde $x,y>0$. Encontre a função de distribuição de $X$.
</div>

<div class='alert alert-success'>
**Proposição 6.** Seja $\textbf{X}$ 
 um vetor de variáveis aleatórias contínuas. A função de densidade marginal de $X_i$
 é dada por
$$f_{X_i}(x_i)=\int_{\mathbb{R}^{n-1}}f(\textbf{x})d\textbf{x}_{(−i)}.$$
</div>
...
$$f(x,y)=8xy,$$
com $0<x<y<1.$ Determine as densidades marginais de $X$
 e $Y$.


**Solução**

$$f_X(x)=\int_{\mathbb{R}}f(x,y)dy=8x\int_x^1 ydy=4x(1−x^2),$$
para $0<x<1$ e

$$f_Y(y)=\int_{\mathbb{R}}f(x,y)dx=8y\int_0^y xdx=4y^3,$$
para $0<y<1$.
</div>

<div class='alert alert-warning'>
**Exercício 9.** Considere a seguinte função de densidade conjunta,
$$f(x,y)=xy^2e^{−y(x+1)}.$$
...
# 3. Covariância, correlação e esperança condicional
...
Agora, sem perda de generalidade, assuma que $X$ e $Y$ são variáveis aleatórias discretas com média zero, assumindo os valores $x_1,\ldots,x_n$ e $y_1,\ldots,y_m$, respectivamente. 

Defina o produto interno ponderado

$$\langle \textbf{x},\textbf{y}\rangle_P=\sum_{i=1}^n\sum_{j=1}^m x_iy_jP(X=x_i,Y=y_j)=E(XY).$$
Observe que

$$||\textbf{x}||_P=\sqrt{\langle \textbf{x},\textbf{x}\rangle_P}=\sqrt{E(X^2)}=\sqrt{Var(X)},$$
onde a última igualdade é válida porque $E(X)=0$. Essa quantidade, conhecida como desvio padrão, é interpretada como a norma do vetor $\textbf{x}$. Para o caso no qual $E(X)=\mu$, o desvio padrão é a norma do vetor centrado $\tilde{\textbf{x}}=(x_1-\mu,\ldots,x_n-\mu)$. Além disso, podemos definir $E(XY)$ como o produto interno ponderado independente da naturaza discreta de $X$ ou $Y$. Podemos então definir a covariância.
...
Na prática, a covariância dá a relação entre as variáveis $X$ e $Y$: (a) Se $Cov(X,Y)<0$, então espera-se que o aumento(decrescimento) de $X$ implique em um decrescimento(aumento) de $Y$, (b) se $Cov(X,Y)>0$, então espera-se que o aumento(decrescimento) de $X$ implique em um aumento(decrescimento) de $Y$ e (c) $Cov(X,Y)=0$ implica na ausência de relação linear entre $X$ e $Y$.
...
::: {#exm-}
Seja $(X,Y)$ um vetor de variáveis aleatórias com função de probabilidade conjunta dada por ... 
:::
...
<div class='alert alert-warning'>
**Exercício** Suponha que $X$ e $Y$ assumem valores em $\{−1,0,1\}$
...
1. Considere a densidade conjunta
...
2. Verifique
  ...
  c. $Cov(aX,Y)=Cov(X,aY)=aCov(X,Y)$, a∈R.

Conclua que $Cov(aX,bY)=abCov(X,Y)$, a,b∈R.

3. Sejam $X_1,\ldots,X_n$ e $Y_1,\ldots,Y_m$ variáveis aleatórias. Mostre que
...
4. Sejam $X,Y$ duas variáveis aleatórias. Mostre que
