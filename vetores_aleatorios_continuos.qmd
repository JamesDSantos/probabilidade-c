...
a) verifique que $f(x,y)$ é de fato uma função densidade conjunta .

b) encontre a função densidade marginal de $X$.
...
::: {#exr-}
Para $n>0$  inteiro, suponha que $X\sim\hbox{Gama}(n+1,x)$. Além disso, suponha que dado $x$, $Y_1,\ldots,Y_n$
 são variáveis aleatórias independentes que possuem a seguinte densdidade condicional:

$$f(y_i|x)=\frac{1}{x},$$
para $0<y_i<x$.

a) Determine a densidade marginal do vetor $(Y_1,\ldots,Y_n)$

b) Determine a densidade condicional de $X$ dado os valores de $Y_1,\ldots,Y_n$
:::


## Função de vetores aleatórios contínuos

### Caso para uma variável
...
::: {#exr-}
Seja $X$ uma variável aleatória com distribuição Uniforme no intervalo $(0, 1)$, ou seja, sua função de densidade de probabilidade é:
$$f_X(x) = I_{(0,1)}(x).$$
Encontre a função densidade de $Y = -\ln(X)$.
:::

### Caso vetorial com $g$ invertível
...
### Caso vetorial com $g:\mathbb{R}^n\rightarrow\mathbb{R}^n$, com $n>m$
...
Em seguida, podemos encontrar a densidade marginal de $\textbf{Y}$
 integrando a conjunta acima em $\textbf{W}$:

$$f_\textbf{Y}(\textbf{y})=\int f_\textbf{X}(g_*^{−1}(\textbf{y},\textbf{w}))|\det\mathcal{J}g_*^{-1}(\textbf{y},\textbf{w})|d\textbf{w}.$$
::: {#exm-}
**Soma de duas variáveis aletórias contínuas independentes.** Sejam $X_1,X_2$ variáveis aleatórias independentes e considere o problema de encontrar a função densidade de $Y=X_1+X_2$.
...
$$f_{Y,W}(y,w)=f_{X_1}(y-w)f_{X_2}(w)$$
e a função densidade desejada é
...
::: {#exr-}
Sejam $X_1$ e $X_2$ variáveis aleatórias independentes com distribuição Exponencial(1), cuja função densidade é 
...
::: {#exr-}
Sejam $X_1$ e $X_2$ variáveis aleatórias independentes com distribuição Uniforme(0,1), cuja função densidade é
...
a) Sejam $X_1,X_2$ variáveis aleatórias independentes com $X_i\sim\hbox{Normal}(\mu_i,\sigma^2_i)$. Mostre que $Y=X_1+X_2\sim\hbox{Normal}(\mu_1+\mu_2,\sigma_1^2+\sigma_2^2)$.

b) Sejam $X_1,\ldots,X_n$ variáveis aleatórias independentes com $X_i\sim\hbox{Normal}(\mu_i,\sigma^2_i)$. Mostre por indução que $Y=X_1+\cdots+X_n\sim\hbox{Normal}(\sum_{i=1}^n \mu_i,\sum_{i=1}^n \sigma^2_i)$
:::

::: {#exr-}
**Distribuição Chi Quadrado.** A função densidade da distribuição $\chi^2_\nu$ é 

$$f(x) = \frac{1}{2^{\nu/2} \Gamma(\nu/2)} x^{\nu/2 - 1} e^{-x/2}I_{(0,\infty)}(x),$$
onde $\nu>0$.

a) Sejam $X_1,X_2$ variáveis aleatórias independentes com $X_i\sim\chi^2_{\nu_i}$. Mostre que $Y=X_1+X_2\sim\chi^2_{\nu_1+\nu_2}$.

b) Sejam $X_1,\ldots,X_n$ variáveis aleatórias independentes com $X_i\sim\chi^2_{\nu_i}$. Mostre, por indução, que $Y=\sum_{i=1}^{n}X_i\sim\chi^2_{\sum_{i=1}^n \nu_i}$.

c) Sejam $Z_i\sim\hbox{Normal}(0,1)$ variáveis independentes. Sabendo que $Z_i^2\sim\chi^2_1$, qual é a distribuição de $Z_1^2+\cdots+Z_n^2?$
:::

::: {#exr-}
**Distribuição t-Student.** Sejam $X\sim\hbox{Normal}(0,1)$ e $Y\sim\chi^2_{\nu}$ variáveis independentes. Mostre que a função densidade de 
...
::: {#exr-f-snedecor}
**Distribuição F de Snedecor.** Sejam $U$ e $V$ variáveis aleatórias independentes tais que $U \sim \chi^2_{\nu_1}$ e $V \sim \chi^2_{\nu_2}$. A distribuição F de Snedecor com $\nu_1$ e $\nu_2$ graus de liberdade é definida pela transformação:$$W = \frac{U/\nu_1}{V/\nu_2}$$

a) Para encontrar a densidade de $W$, defina uma variável auxiliar $Z = V$. Encontre a densidade conjunta $f_{W,Z}(w, z)$ utilizando o método do Jacobiano.

b) Mostre que a densidade marginal de $W$ é dada por:$$f_W(w) = \frac{\Gamma\left(\frac{\nu_1+\nu_2}{2}\right)}{\Gamma\left(\frac{\nu_1}{2}\right)\Gamma\left(\frac{\nu_2}{2}\right)} \left(\frac{\nu_1}{\nu_2}\right)^{\frac{\nu_1}{2}} \frac{w^{\frac{\nu_1}{2}-1}}{\left(1 + \frac{\nu_1}{\nu_2}w\right)^{\frac{\nu_1+\nu_2}{2}}} I_{(0, \infty)}(w)$$

c) Como a distribuição $F$ se relaciona com a distribuição $t$ de Student? (Dica: Observe a definição de $T$ no exercício anterior e considere o que ocorre com $T^2$).:::
